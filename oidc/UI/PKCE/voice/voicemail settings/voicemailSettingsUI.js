...
document.getElementById('getDefaultGreetingContentMp3').addEventListener("click", () =>{ 
    getGreetingContent(localStorage.getItem('access_token'), "mp3", 0); 
});

document.getElementById('getDefaultGreetingContentOgg').addEventListener("click", () =>{
     getGreetingContent(localStorage.getItem('access_token'), "ogg", 0);
});

document.getElementById('getCustomGreetingContentMp3').addEventListener("click", () =>{ 
    getGreetingContent(localStorage.getItem('access_token'), "mp3", 1); 
});
document.getElementById('getCustomGreetingContentOgg').addEventListener("click", () =>{
    getGreetingContent(localStorage.getItem('access_token'), "ogg", 1); 
});

document.getElementById('uploadGreetingContent').addEventListener("click", () =>{ 
    uploadGreetingContent(localStorage.getItem('access_token')); 
});

document.getElementById('getUserSettings').addEventListener("click", () =>{ 
    getUserSettings(localStorage.getItem('access_token')); 
});

document.getElementById('getVoicemailUsage').addEventListener("click",async () =>{ 
    let res =await getVoicemailUsage(localStorage.getItem('access_token')); 
    log(res);
});

document.getElementById('resetGreetingContent').addEventListener("click", () =>{
    resetGreetingContent(localStorage.getItem('access_token')); 
});
